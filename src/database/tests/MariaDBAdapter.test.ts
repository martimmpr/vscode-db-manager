...
describe('MariaDBAdapter', () => {
    ...
    describe('Initialization', () => {
        test('should initialize as MariaDB adapter', () => {
            adapter = new MariaDBAdapter(createConfig());
            expect(adapter).toBeInstanceOf(MariaDBAdapter);
        });

        test('should extend MySQLAdapter', () => {
            adapter = new MariaDBAdapter(createConfig());
            // MariaDB inherits from MySQL
            expect(adapter.constructor.name).toBe('MariaDBAdapter');
        });

        test('should initialize with custom database', () => {
            adapter = new MariaDBAdapter(createConfig('mydb'));
            expect(adapter).toBeInstanceOf(MariaDBAdapter);
        });

        test('should create connection with correct configuration', async () => {
            adapter = new MariaDBAdapter(createConfig('testdb'));
    ...
    describe('MariaDB Specific Features', () => {
        ...
        test('should support MariaDB specific types', async () => {
            mockQuery.mockResolvedValue([{}, []]);
            
            await adapter.createTable('mydb', 'test', [
                { name: 'id', type: 'SERIAL', constraints: [] },
        ...
        test('should handle sequences like MySQL AUTO_INCREMENT', async () => {
            mockQuery.mockResolvedValue([{}, []]);
            
            await adapter.createTable('mydb', 'test', [
                { name: 'id', type: 'INT', constraints: ['GENERATED ALWAYS AS IDENTITY'] }
            ]);
